...
test("runScan flags awaited leaks but not NEXT_PUBLIC_* reads", () => {
  const result = runScan([ASYNC_PUBLIC, "--json"], process.cwd());
  assert.equal(result.code, 1);
  const boundary = result.findings.filter(
    (finding) => finding.ruleId === "rsc/client-boundary-prop",
  );
  assert.equal(boundary.length, 2, JSON.stringify(result.findings, null, 2));
  assert.ok(
